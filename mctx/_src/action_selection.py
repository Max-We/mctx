...
def muzero_puct_new_action_selection(
    rng_key: chex.PRNGKey,
    tree: tree_lib.Tree,
    node_index: chex.Numeric,
    depth: chex.Numeric,
...
def muzero_uct_action_selection(
    rng_key: chex.PRNGKey,
    tree: tree_lib.Tree,
    node_index: chex.Numeric,
    depth: chex.Numeric,
  ...
  node_visit = tree.node_visits[node_index]
  # uct
  uct_score = 1.25 * jnp.sqrt(jnp.log(node_visit) / (visit_counts + 1))
  prior_logits = tree.children_prior_logits[node_index]
  prior_probs = jax.nn.softmax(prior_logits)
  # p * uct
  policy_score = prior_probs * uct_score
  chex.assert_shape([node_index, node_visit], ())
  chex.assert_equal_shape([prior_probs, visit_counts, policy_score])
  # value
  value_score = qtransform(tree, node_index)
  ...
  uct_tuned_score = jnp.sqrt(jnp.log(node_visit) / (visit_counts + 1) *
                             jnp.minimum(
                               ...
                               variance_score + jnp.sqrt(
                                 (2 * jnp.log(node_visit))/ (visit_counts + 1)
                               )
                             ))
  prior_logits = tree.children_prior_logits[node_index]
  prior_probs = jax.nn.softmax(prior_logits)
...
def muzero_uct_tuned_new_h_action_selection(
    rng_key: chex.PRNGKey,
    tree: tree_lib.Tree,
    node_index: chex.Numeric,
    depth: chex.Numeric,
...
def muzero_uct_bayes_action_selection(
    rng_key: chex.PRNGKey,
    tree: tree_lib.Tree,
    node_index: chex.Numeric,
    depth: chex.Numeric,
...
def muzero_uct_v_new_h_action_selection(
    rng_key: chex.PRNGKey,
    tree: tree_lib.Tree,
    node_index: chex.Numeric,
    depth: chex.Numeric,
